...
class UniteCreatorDialogParamWork{
	...
	private function putTab($paramType, $isSelected = false, $isSelect = false){
		...
		if($isSelect == true):
		?>
			<option <?php echo UniteProviderFunctionsUC::escAddParam($selectHtml)?> data-type="<?php echo esc_attr($paramType)?>" value="<?php echo esc_attr($contentID)?>" >
				<?php _e($title, "unlimited_elements")?>
			</option>
		<?php
		else:
		?>
			<a href="javascript:void(0)" data-type="<?php echo esc_attr($paramType)?>" data-contentid="<?php echo esc_attr($contentID)?>" class="<?php echo esc_attr($class)?>">
				<?php _e($title, "unlimited_elements")?>
			</a>
		<?php
		endif;
